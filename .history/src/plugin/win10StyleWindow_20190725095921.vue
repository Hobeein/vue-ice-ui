...
<template>
  <transition
    :enter-active-class="{ animateCss, zoomIn: isZoom, zoomInUp: isZoomDown }"
    :leave-active-class="{ animateCss, zoomOut: isZoom, zoomOutDown: isZoomDown }"
  >
    <div
      v-show="windowShow"
...
const win10StyleWindow = {
    ...
    methods: {
        animateZoomChange(status) {
            if(status === 'mini'){
                this.isZoom = false
                this.isZoomDown = true
            }else if(status === 'miniRe'){
                this.isZoom = true
                this.isZoomDown = false
            }
        },
        afterCloseClick(){
